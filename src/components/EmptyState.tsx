...
export function EmptyState({ completed = false }: { completed?: boolean }) {
  return (
    <div className="surface relative grid min-h-80 place-items-center overflow-hidden rounded-[2rem] px-6 py-10 text-center">
      <Image alt="" className="prism-only absolute inset-0 h-full w-full object-cover opacity-[.12]" fill sizes="80vw" src="/prism-abstract.png" />
      <div className="relative z-10">
        <div className="prism-only mx-auto h-28 w-48 overflow-hidden rounded-2xl border border-white/10"><Image alt="Detalle de una composición abstracta de diseño" className="h-full w-full object-cover" height={112} src="/prism-abstract.png" width={192} /></div>
        <CapiCharacter mood={completed ? "party" : "calm"} className="non-prism-only mx-auto h-40" />
